...
const firstLast = people.map((person) => {
    const values = person.split(', ')
    const firstLast = values[1] + ' ' + values[0];
...
const isEveryone19OrOlder = devs.every((devOver) => {
    if (devOver.year < 2007)
        return true
})
...
const totalYearsLived = inventors.reduce((accumulator, currentValue) => {
    const yearsLived = currentValue.passed - currentValue.year;
...
const travelMethodCounts = travelMethods.reduce((counter, element) => {
